...
#   np.savetxt('tdiff', data)
#   1/o

    fig, ax = plt.subplots(1, tight_layout=True)
    ax.plot(data[:, 1], data[:, 2])
    ax.set_ylim([1e-3, 0.03])
    ax.set(ylabel=r'$\frac{\|\frac{T^n-T^{n-1}}{\Delta t}\|_{L^2}}{|\Omega|^{1/2}}$',
           xlabel=r'$t$',
           title='Rate of Change in Temperature History')
    fig.savefig('tdiff_his')
    return
