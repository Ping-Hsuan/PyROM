...
if T0 == 1:
    files_dict = setup.create_dict(filenames, '^.*_ic_h10_.*_(\dp\d+)_dual_norm$')
elif T0 >= 1:
    files_dict = setup.create_dict(filenames, '^.*_zero_h10_.*_(\dp\d+)_dual_norm$')
...
ax.set(ylabel=r'$\triangle(\theta_g;{\theta^*_g}='+str(int(anchor))+r')$',
       xlabel=r'\# percentage filtered', ylim=[1e-2, 1],
