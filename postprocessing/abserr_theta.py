...
ax.plot(data[:, 0], data[:, 1], 'b-o', mfc="None")
ax.plot(data[:, 0], data[:, 2], 'k-o', mfc="None")

ax.set_title(r'Absolute error in the mean flow at $\theta_g='+str(int(deg)+90)+'$')
ax.set_ylabel(r'$\|u_{avg} - \widehat{u}_{avg}\|_{H^1}$')
ax.set_xlabel(r'$N$')
ax.set_xlim([0, max(data[:, 0])])
#ax.legend(loc=0)

print("---------------------------------------------")
fig.savefig(tpath+'abserr_theta_'+str(int(deg)+90)+'.png')
np.savetxt(tpath+'N_list_'+str(int(deg)+90)+'.dat', data[:, 0])
np.savetxt(tpath+'rom_abserr_theta_'+str(int(deg)+90)+'.dat', data[:, 1])
np.savetxt(tpath+'proj_abserr_theta_'+str(int(deg)+90)+'.dat', data[:, 2])
print("---------------------------------------------")
