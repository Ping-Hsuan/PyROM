...
p1 = subprocess.Popen('ls'.split(), stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
filenames = (p1.stdout.read()).decode().split('\n')[:-1]
p1.wait()
...
fig.savefig('online.png')
np.savetxt('angle.dat', data[:, 0])
np.savetxt('erri.dat', data[:, 1])
