...
target_dir = '/vel_mabserr/'
setup.checkdir(target_dir)

search_dir = './'+model+'_info/vel_mabserr'
if model == 'all':
...
domain = np.sqrt(40*np.sin(data[:, 0]*np.pi/180))
...
ax.set(xlabel=r'$\theta_g$', ylabel=r'$\|u(\theta_g) -' +
       ...
       title='Absolute error in the mean velocity with \n ROM anchor at ' +
...
print("---------------------------------------------")
fig.savefig('.'+target_dir+'vel_abserr_N'+N+'_'+mode+'.png')
np.savetxt('.'+target_dir+'angle_list_'+mode+'.dat', data[:, 0])
np.savetxt('.'+target_dir+'vel_rom_abserr_N'+N+'_'+mode+'.dat', data[:, 1])
np.savetxt('.'+target_dir+'vel_proj_abserr_N'+N+'_'+mode+'.dat', data[:, 2])
print("---------------------------------------------")
