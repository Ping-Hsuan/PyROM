...
if sc == 'rom':
    ...
    sc_dir = './rom_norm/'
    scale = np.loadtxt(sc_dir+'rom_T_h1norm_N'+N+'_'+mode+'.dat')
    ylb = r'$\frac{\triangle_T(\theta_g)}{\|\langle T_{ROM}(\theta_g) \rangle_g\|_{H^1}}$'
elif sc == 'fom':
    ...
    tmp = np.loadtxt(sc_dir+'fom_T_h1norm.dat')
    scale = tmp[idx]
    ylb = r'$\frac{\triangle_T(\theta_g)}{\|\langle T_{FOM}(\theta^*_g) \rangle_g\|_{H^1}}$'
elif sc == 'romabserr':
    # scaled with the rom_abserr
    # This is essentially the effectivity
    sc_dir = './temp_mabserr/'
    scale = np.loadtxt(sc_dir+'temp_rom_abserr_N'+N+'_'+mode+'.dat')
    print(scale)
    ylb = r'$\frac{\triangle_T(\theta_g)}{\|\langle T_{FOM}-T_{ROM} \rangle_g\|_{H^1}}$'
elif sc == 'eta':
    # scaled with the rom_abserr
    # This is essentially the effectivity
    sc_dir = './temp_mabserr/'
    abserr = np.loadtxt(sc_dir+'temp_rom_abserr_N'+N+'_'+mode+'.dat')
    angle = np.loadtxt('./temp_dual_norm/angle_list_'+mode+'.dat')
    dual = np.loadtxt('./temp_dual_norm/temp_erri_N'+N+'_'+mode+'.dat')
    anchor = setup.find_anchor()
    idx = np.where(angle == int(anchor))
    ylb = r'$\frac{\triangle_T(\theta_g)}{\eta_T(\theta^*_g)}$'
    effect = dual/abserr
    scale = effect[idx]
    print(scale)
    ylims = [1e-1, 1e3]
    ...
    anchor = setup.find_anchor()
    idx = np.where(angle == int(anchor))
    ylb = r'$\frac{\triangle_T(\theta_g)\|\langle T_{ROM}(\theta^*_g) \rangle_g\|}{\eta_T(\theta^*_g)}$'
    effect = dual/abserr
    sc_dir = './rom_norm/'
    rom_norm = np.loadtxt(sc_dir+'rom_T_h1norm_N'+N+'_'+mode+'.dat')
    print(abserr)
    print(dual)
    print(effect)
    print(rom_norm)
    ...
    anchor = setup.find_anchor()
    idx = np.where(angle == int(anchor))
    ylb = r'$\frac{\triangle_T(\theta_g)\|\langle T_{ROM}(\theta_g) \rangle_g\|}{\eta_T(\theta^*_g)}$'
    effect = dual/abserr
    sc_dir = './rom_norm/'
    rom_norm = np.loadtxt(sc_dir+'rom_T_h1norm_N'+N+'_'+mode+'.dat')
    print(abserr)
...
print("---------------------------------------------")
fig.savefig('.'+target_dir+'temp_dual_norm_N'+N+'_sc_'+sc+'.png')
np.savetxt('.'+target_dir+'temp_erri_N'+N+'_sc_'+sc+'.dat', data[:, 1])
print("---------------------------------------------")
