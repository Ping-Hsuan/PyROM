def temp_mabserr_wparam(model, N, T0, mode, idx, aval, tkey, tval, fd=None):
    import numpy as np
    ...
    data = rdatatoarray(dict_final, 'mabserr')

    solver = model.upper()
    plot_params1 = set_pltparams('rom_mabserr', solver, N, T0, fd)
    plot_params2 = set_pltparams('proj_mabserr', solver, N, T0, fd)

    fig, ax = plt.subplots(1, tight_layout=True)
    set_ax(ax, 'temp_mabserr', tkey, tval)
    ...
    header = tkey[0]+','+'rom_mabserr'+','+'proj_mabserr'
    mysave(fig, target_dir, data, 'mabserr', header, N, fd)


if __name__ == '__main__':
    import sys
    temp_mabserr_wparam(*sys.argv[1:])
