...
if min(4, nb) == 1:
    ...
    ax.plot(np.linspace(500, 1000, K), fomt[i+1, :], 'k-', mfc="None", label='Snapshot')
    ...
    ax.hlines(y=ta[i], xmin=t_grid[i, 0], xmax=t_grid[i, -1], colors='b', linestyle='--', label='ROM avg')
    ...
    for i in range(min(4, nb)):
        ...
        axs[i].plot(np.linspace(500, 1000, K), fomt[i+1, :], 'k-', mfc="None", label='Snapshot')
        ...
        axs[i].hlines(y=ta[i], xmin=t_grid[i, 0], xmax=t_grid[i, -1], colors='b', linestyle='--', label='ROM avg')
