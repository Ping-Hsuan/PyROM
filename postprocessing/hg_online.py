...
#   mdual_2nd.append(max(erri_opt[5:]))
    ...
    #                                                         data[2][:itr], model, mode)
        ...
        opt_merr_nu, opt_m_nu, opt_std_nu = \
            online.get_optqoi(merr_all, m_all, sd_all,
...
#       mnurelerr_2nd.append(max(opt_merr_nu[5:]))
        ...
        ax3.set(xlabel=r'$'+features['Param']+'$', ylabel=r'$\langle \textrm{Nu} \rangle_s$',
        ...
        np.savetxt('nu_merr'+pt+'_'+str(ncand)+'_itr'+str(itr)+'.csv', np.column_stack((angle, opt_merr_nu)), delimiter=',', header=features['Param']+', nu_merr', comments="")
        np.savetxt('nu_m'+pt+'_'+str(ncand)+'_itr'+str(itr)+'.csv', np.column_stack((angle, opt_m_nu)), delimiter=',', header=features['Param']+', nu_m', comments="")
        np.savetxt('nu_std'+pt+'_'+str(ncand)+'_itr'+str(itr)+'.csv', np.column_stack((angle, opt_std_nu)), delimiter=',', header=features['Param']+', nu_std', comments="")
    ...
    if (onlfeature['mrelerr_h1']):
        ...
        ax5.set(xlabel=r'$'+features['Param']+'$', ylabel=r'$\frac{\|\langle \bf{u} - \bf{\tilde{u}} \rangle\|_{H^1}}{\|\langle \bf{u} \rangle \|_{H^1}}$',
                ylim=[0, 0.7], xticks=features['Ptrain'])
...
#       ax5.set_ylim([10**ylim_exp, None])
        ...
        ax6.set(xlabel=r'$'+features['Param']+'$', ylabel=r'$\frac{\|\langle \bf{u} - \bf{\tilde{u}} \rangle\|_{H^1}}{\|\langle \bf{u} \rangle \|_{H^1}}$',
                ylim=[0, 0.1], xticks=features['Ptrain'])
        for k in P_train[:-1]:
            idx = list(angle).index(k)
...
#       ax6.set_ylim([10**ylim_exp, None])
        ...
        np.savetxt('rom_fldrelerr'+pt+'_'+str(ncand)+'_itr'+str(itr)+'.csv', np.column_stack((angle, opt_flderr_rom)), delimiter=',', header=features['Param']+', rom_fldrelerr', comments="")
        np.savetxt('proj_fldrelerr'+pt+'_'+str(ncand)+'_itr'+str(itr)+'.csv', np.column_stack((angle, opt_flderr_rom)), delimiter=',', header=features['Param']+', proj_fldrelerr', comments="")
